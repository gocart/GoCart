...
<?php endif; ?>
            ...
            function get_customer_form()
            {
                ...
                $('#shipping_payment_container').html('<div class="checkout_block"><img alt="loading" src="/assets/images/ajax-loader.gif"/><br style="clear:both;"/></div>').hide();
            ...
            var logged_in_user = <?php if ($this->Customer_model->is_logged_in(false, false))
    echo "true"; else
    echo "false"; ?>;

            var shipping_required = <?php echo ($this->go_cart->requires_shipping()) ? 'true' : 'false'; ?>;
            var shipping = Array();
            var shipping_choice = '<?php $shipping = $this->go_cart->shipping_method();
if ($shipping)
    echo $shipping['method']; ?>';

            var addr_context = '';
            var ship_to_bill_address = <?php if (isset($customer['ship_to_bill_address'])) {
    echo $customer['ship_to_bill_address'];
} else {
    echo 'false';
} ?>;
            var addresses;
...
<div class="checkout_block">
    <div id="customer_info_fields">
        ...
        <img alt="loading" src="/assets/images/ajax-loader.gif"/>
    </div>
    <br style="clear:both;"/>
</div>
