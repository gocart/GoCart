...
(function($){
	...
	var Redactor = function(element, options)
	{
		...
		this.opts = $.extend({
			...
			modal_image_edit: String() +
				...
				'<label><input type="checkbox" id="redactor_file_link_blank"> ' + RLANG.link_new_tab + '</label>' +
	...
	Redactor.prototype = {
		...
		imageEdit: function(e)
		{
			...
			var callback = $.proxy(function()
			{
				...
				if ($(parent).get(0).tagName === 'A')
				{
					...
					{
						$('#redactor_file_link_blank').attr('checked', true);
					}
				}
		...
		imageSave: function(el)
		{
			...
			if (link !== '')
			{
				var targettext = ' target="_self"';
				var target = '_self';
				if ($('#redactor_file_link_blank').prop('checked'))
				{
				...
				if ($(parent).get(0).tagName !== 'A')
				{
					$(el).replaceWith('<a href="' + link + '" ' + targettext + '>' + this.outerHTML(el) + '</a>');
				}
				else
				{
